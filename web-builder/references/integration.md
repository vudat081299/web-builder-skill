...
| `wb-shell` + `__body` / `__side` / `__main` (the page frame) | **no library.** The rail's sticky column, its own scroll, and the off-canvas drawer + scrim under 900px all ship in CSS. Your app owns exactly one thing: toggling `is-side-collapsed` (desktop) / `is-side-open` (narrow) on the shell, plus closing on a nav-link click or an outside tap. In React that's one `useState` + a `useMediaQuery("(max-width: 900px)")` — **keep that breakpoint in step with the CSS**. Content column = `wb-container wb-container--pad` |
| `wb-page-head` / `wb-section` / `wb-block` (page rhythm) | no JS — nest them in order and the vertical spacing is the shipped one. Tune globally with `--wb-section-gap` / `--wb-block-gap` / `--wb-page-pad-*`; do **not** reach for `mt-*` / `margin-top` per screen |
| `wb-app` (the document baseline) | no JS — a class on `<body>`. In React/Next put it on the root layout's `<body className="wb-app">`; `.wb-shell` carries the same declarations, so a shelled route needs nothing more, but a **shell-less** route (auth, landing, an embedded widget) will render browser-default serif with blue links without it. **Tailwind users:** Preflight also resets box-sizing and links, and whichever stylesheet loads last wins — load `web-builder.css` after Tailwind, or drop `.wb-app` and let Preflight be the baseline (the components are border-box either way, scoped by the library) |
| `wb-shell__side-toggle` (the ☰) | no JS beyond the same shell toggle above. Purely a visibility class: it appears at the 900px fold and hides above it, so you never write a media query to match the rail's state |
| `wb-navbar` / `wb-nav` / `wb-sidenav` (app-shell navigation) | plain markup + classes; drive `.is-active` from your **router** (React Router `NavLink`). The mobile rail is the shell's job (row above), not yours; `wb-navbar`'s own inline-links→☰ collapse is a container query, also free |
